...
parser.add_argument('--add_value', dest='add_value', type=float, help='value to add')
parser.add_argument('--weight_name', dest='weight_name', type=str, help='name of weights')
args = parser.parse_args()

value_to_add = args.add_value
weight_name = args.weight_name
base_path = '/content/drive/MyDrive/Workspace'
results_path = f"{base_path}/results/{weight_name}/before"
...
if not os.path.exists(f"{base_path}/results/{weight_name}/before"):
   os.mkdir(f"{base_path}/results/{weight_name}/before")
...
print("################################Loading Base Weight################################")

model_path = f"{base_path}/Modeling-Autism/FullyTrainedModelWeights.h5"
...
print(f"################################Changing weights to +{value_to_add} ################################")

for layer in mr_model.keras_model.layers:
  #  or layer.name[:10]=='mrcnn_bbox'
  if (layer.name=='d_lbl3' or layer.name=='d_l3' or layer.name[:10]=='mrcnn_bbox') and len(layer.get_weights())!=0:
    print(color.PURPLE + color.BOLD + str(layer.name) + color.END + color.END )
    ws = layer.get_weights()[0].shape
    bs = layer.get_weights()[1].shape
    # w = np.zeros(shape=ws)
    # b = np.zeros(shape=bs)
    # w = np.ones(shape=ws)
    # b = np.ones(shape=bs)
        # w = np.random.rand(ws[0],ws[1])
    # b = np.random.rand(bs[0])
    print("Previous Weights:\n",layer.get_weights())
    rand_float = random.uniform(0.02,0.03)
    # rand_float = random.uniform(0.02,0.03)
    # rand_float = random.uniform(0.04,0.05)


    print(rand_float)
    w = layer.get_weights()[0] + value_to_add
    b = layer.get_weights()[1] + value_to_add
    weights = [w,b]
    layer.set_weights(weights)
    # print("Current Weights:\n",layer.get_weights())

print(f"################################Saving weights before training to {weight_name}################################")
...
print(f"################################Loading weights for inf model from {weight_name}################################")
model_path = f'{base_path}/Modeling-Autism/AllDistanceLayerWieghts_{weight_name}-BeforeTraining.h5'
# mr_model.keras_model.save_weights(model_path)
mr_model_inf = modellib.MaskRCNN(mode="inference", config=config, model_dir='./')
mr_model_inf.load_weights(model_path, by_name=True)
